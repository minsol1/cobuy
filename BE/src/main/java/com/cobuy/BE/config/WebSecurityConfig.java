...
import com.cobuy.BE.filter.JwtAutheticationFilter;
import com.cobuy.BE.handler.OAuth2SuccessHandler;
...
//
//        http.csrf(csrf -> csrf.disable()).authorizeHttpRequests(auth -> auth.requestMatchers("/v2/api-docs", "/v3/api-docs", "/v3/api-docs/**", "/swagger-resources",
//                "/swagger-resources/**", "/configuration/ui", "/configuration/security", "/swagger-ui/**",
//                "/webjars/**", "/swagger-ui.html").permitAll().anyRequest().permitAll());
//        return http.build();
//    }
...
//                .authorizeHttpRequests(request -> request.requestMatchers("/", "/api/v1/auth/**", "/oauth2/**","/swagger-ui/**", "/error").permitAll() // 이 경로는 인증 없이 접근 허용
